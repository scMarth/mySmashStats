// htmlMatchParser.cpp : Defines the entry point for the console application.
//
...
int findMatchID(smatch sm) {
	...
	for (auto x : sm) {
		//cout << "findMatchID: " << (string)x << endl;
		string match_id_str = getSubstringThatMatches("=\'[0-9]+\'", (string)x);
		match_id_str = getSubstringThatMatches("[0-9]+", match_id_str);
		int match_id = 0;
...
void getOpponentInfo(ifstream &infile, string opponent_num) {
